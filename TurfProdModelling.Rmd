...
ts_data <- left_join(pred_data,pred_depth, by='site') %>%
  ...
  filter(subs_group %in% c('algal_turf','macroalgae', 'halimeda')) %>%
...
ggplot(data=fts_data) +
  ...
  geom_smooth(aes(x=Year,y=turf_prod_mean), 
...
### And using Duarte's et al 2022's data to explore a model for macroalgae and halimeda?
```{r}

dmachal <- read.csv('macroalgae_npp_dataset.csv') %>%
  filter(Habitat_category_Duarte_etal_2021 %in% c('Coral Reef Algae','Halimeda bioherme ','Subtidal brown algae') & abs(Latitude_decimal_degrees) < 30)




## How many and what are the available values for Halimeda first

ggplot(dmachal %>% filter(Genus == 'Halimeda')) +
  geom_point(aes(x=Depth_max_m, y=Avg_NPP_kg_C_m2_y))

```
```{r}
p1 <- ggplot(data=ts_data) +
  geom_line(aes(x=Year,y=macroalgae,group=interaction(Transect,site))) +
  geom_smooth(aes(x=Year,y=macroalgae), 
              method = 'loess', formula = y ~ x,
              colour='orange', alpha=0.8) +
  scale_x_continuous(breaks=c(2005,2009,2013,2017)) +
...
p2 <- ggplot(data=ts_data) +
  geom_line(aes(x=Year,y=halimeda,group=interaction(Transect,site))) +
  geom_smooth(aes(x=Year,y=halimeda), 
              method = 'loess', formula = y ~ x,
              colour='orange', alpha=0.8) +
  scale_x_continuous(breaks=c(2005,2009,2013,2017)) +
  ylab('Halimeda cover') +
