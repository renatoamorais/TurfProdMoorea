...
fts_data <- cbind(fts_data,
      macr_prod_mean=apply(tot_macr_prod,1,median),
      macr_prod_lhdi=apply(tot_macr_prod,1,function(x) median_hdci(x)$ymin),
      macr_prod_uhdi=apply(tot_macr_prod,1,function(x) median_hdci(x)$ymax))
...
ggplot(data=fts_data) +
  geom_line(aes(x=Year,y=macr_prod_mean,group=interaction(Transect,site))) +
  geom_smooth(aes(x=Year,y=macr_prod_mean), 
              method = 'loess', formula = y ~ x,
              colour='orange', alpha=0.8) +
  scale_x_continuous(breaks=c(2005,2009,2013,2017)) +
